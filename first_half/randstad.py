...
class randstad(scrapy.Spider):
    name = 'randstad'
    page_number = 1
    start_urls = ['https://www.randstad.gr/en/jobs/']
    # mysql connection
    ...
    def parse(self, response):
        # find job path
        domain = 'https://www.randstad.gr'
        page_posts= response.xpath("//header[@class='cf']//a/@href").extract()
        get_next_page =response.xpath("//a[@id='ctl06_ctl06_NextPageHyperLink']/@href").extract_first()
        for i in range(len(page_posts)):
            yield response.follow(domain+page_posts[i], callback=self.parse_job)
        if len(get_next_page) is not None:
            yield response.follow(domain+get_next_page, callback=self.parse)
    def parse_job(self, response):
        ...
        print(("test",len(url_from_database)))
        if len(url_from_database) < 1:
            try:
                ...
                print("Randstand, New entries :", self.count_new, "& job url", job_url)
            except mysql.connector.Error as error:
